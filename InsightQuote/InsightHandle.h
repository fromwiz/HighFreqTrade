...
class InsightHandle : public MessageHandle
{
  public:
    ...
    InsightHandle(const std::string &forder_name)
        : base_forder_(forder_name), service_value_(0), query_exit_(false), reconnect_(false), login_success_(false),
          reconnect_count_(0), no_connections_(false)
    {
        //验证基础目录，如果不存在则创建创建目录
  ...
  public:
    ...
    void OnMarketData(const com::htsc::mdc::insight::model::MarketData &data)
    {
        ...
        case MD_TICK: { //快照
            ...
            { //股票
                ...
                //写文件
                save_debug_string(base_forder_, data_type, security_type, data.mdstock().htscsecurityid(),
            ...
            { //债券
                ...
                save_debug_string(base_forder_, data_type, security_type, data.mdbond().htscsecurityid(),
            ...
            { //指数
                ...
                save_debug_string(base_forder_, data_type, security_type, data.mdindex().htscsecurityid(),
            ...
            { //基金
                ...
                save_debug_string(base_forder_, data_type, security_type, data.mdfund().htscsecurityid(),
            ...
            { //期权
                ...
                save_debug_string(base_forder_, data_type, security_type, data.mdoption().htscsecurityid(),
            ...
            { //期货
                ...
                save_debug_string(base_forder_, data_type, security_type, data.mdfuture().htscsecurityid(),
            ...
            { //外汇
                ...
                save_debug_string(base_forder_, data_type, security_type, data.mdforex().htscsecurityid(),
            ...
            { //现货
                ...
                save_debug_string(base_forder_, data_type, security_type, data.mdspot().htscsecurityid(),
            ...
            { //权证
                ...
                save_debug_string(base_forder_, data_type, security_type, data.mdwarrant().htscsecurityid(),
        ...
        case MD_TRANSACTION: { //逐笔成交
            if (data.has_mdtransaction())
            {
                ...
                save_debug_string(base_forder_, data_type, security_type, data.mdtransaction().htscsecurityid(),
        ...
        case MD_ORDER: { //逐笔委托
            if (data.has_mdorder())
            {
                ...
                save_debug_string(base_forder_, data_type, security_type, data.mdorder().htscsecurityid(),
                                  data.mdorder().ShortDebugString());
            }
            break;
        }
        case MD_CONSTANT: { //静态信息
            if (data.has_mdconstant())
            {
                std::string security_type = get_security_type_name(data.mdconstant().securitytype());
                save_debug_string(base_forder_, data_type, security_type, data.mdconstant().htscsecurityid(),
        ...
        case MD_KLINE_1D: { //实时数据只提供15S和1MIN K线
            if (data.has_mdkline())
            {
                ...
                save_debug_string(base_forder_, data_type, security_type, data.mdkline().htscsecurityid(),
                                  data.mdkline().ShortDebugString());
            }
            break;
        }
        case MD_TWAP_1MIN: { // TWAP数据
            if (data.has_mdtwap())
            {
                std::string security_type = get_security_type_name(data.mdtwap().securitytype());
                save_debug_string(base_forder_, data_type, security_type, data.mdtwap().htscsecurityid(),
                                  data.mdtwap().ShortDebugString());
            }
            break;
        }
        case MD_VWAP_1MIN:
        case MD_VWAP_1S: { // VWAP数据
            if (data.has_mdvwap())
            {
                std::string security_type = get_security_type_name(data.mdvwap().securitytype());
                save_debug_string(base_forder_, data_type, security_type, data.mdvwap().htscsecurityid(),
        ...
        case AD_FUND_FLOW_ANALYSIS: { //资金流向分析数据
            if (data.has_mdfundflowanalysis())
            {
                ...
                save_debug_string(base_forder_, data_type, security_type, data.mdfundflowanalysis().htscsecurityid(),
                                  data.mdfundflowanalysis().ShortDebugString());
            }
            break;
        }
        case MD_ETF_BASICINFO: { // ETF的基础信息
            if (data.has_mdetfbasicinfo())
            {
                std::string security_type = get_security_type_name(data.mdetfbasicinfo().securitytype());
                save_debug_string(base_forder_, data_type, security_type, data.mdetfbasicinfo().htscsecurityid(),
        ...
        case AD_ORDERBOOK_SNAPSHOT: {
            if (data.has_orderbooksnapshot())
            {
                ...
                save_debug_string(base_forder_, data_type, security_type, data.orderbooksnapshot().htscsecurityid(),
    ...
    void OnPlaybackPayload(const PlaybackPayload &payload)
    {
        ...
        // debug_print("total number=%d, serial=%d, isfinish=%d", stream.totalnumber(), stream.serial(),
        // stream.isfinished()); google::protobuf::RepeatedPtrField<MarketData>::const_iterator it 	=
        // stream.marketdatalist().marketdatas().begin(); while (it != stream.marketdatalist().marketdatas().end()) {
        //	OnMarketData(*it);
        //	++it;
        // }
    }
    ...
    void OnPlaybackStatus(const com::htsc::mdc::insight::model::PlaybackStatus &status)
    {
        ...
        EPlaybackTaskStatus task_status = status.taskstatus();
    ...
    void OnQueryResponse(const ::com::htsc::mdc::insight::model::MDQueryResponse &response)
    {
        ...
        {
            error_print("query response result: FAIL! ERROR INFO[%d,%s]", response.errorcontext().errorcode(),
        ...
        else
        {
            ...
            debug_print("query response total number=%d, serial=%d, isfinish=%d", stream.totalnumber(), stream.serial(),
                        stream.isfinished());
    ...
    void OnGeneralError(const InsightErrorContext &context)
    {
        ...
        // debug_print("------- PARSE message OnGeneralError");
    }
