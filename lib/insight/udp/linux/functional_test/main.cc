...
void test_init()
{
    ...
    // open_heartbeat_trace();   //打开heartbeat_trace，输出心跳日志消息，默认关闭
...
bool test_login()
{
    ...
    backup_list.insert(std::pair<std::string, int>("221.131.138.171", 9242));
    backup_list.insert(std::pair<std::string, int>("221.223.112.140", 9242));
...
void test_playbackToday()
{
    ...
    */
    ...
    while (1)
    {
        if (handle->service_value_ == CANCELED || handle->service_value_ == COMPLETED ||
            handle->service_value_ == FAILED)
        {
...
void test_udp_client()
{
    ...
    if (udp_client->SubscribeAll(interface_ip) != 0)
    {
        error_print("SubscribeAll failed");
        return;
    }
    std::cout << "input any key to quit...>>";
    char a[4096];
...
{
    printf("\n=======================================================================\n");
    printf("usage: cmd user password ip port cert_folder/interface_ip export_folder test_type\n");
    ...
    printf("test_typ(6 test_udp_client \n");
    printf("test_typ(7 test_udp_client_subscribe_type \n");
    printf("=========================================================================\n\n");
    if (argc >= 8)
    {
        user = argv[1];
    ...
    case 7: {
        test_udp_client_subscribe_type();
        break;
    }
    default:
        break;
